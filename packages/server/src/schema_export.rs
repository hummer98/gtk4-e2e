...
use crate::proto::{
    ...
    Info, KeyRequest, KeyResult, PinchRequest, PressRequest, PropertyEventData, SetValueRequest,
    SwipeRequest, TapTarget, TouchDragRequest, TypeRequest, WaitCondition, WaitRequest, WaitResult,
    Waypoint,
};
...
pub fn write_schemas(out_dir: &Path) -> io::Result<()> {
    ...
    write_one::<SetValueRequest>(out_dir, "SetValueRequest")?;
    write_one::<KeyRequest>(out_dir, "KeyRequest")?;
    write_one::<KeyResult>(out_dir, "KeyResult")?;
    write_one::<Waypoint>(out_dir, "Waypoint")?;
    write_one::<TouchDragRequest>(out_dir, "TouchDragRequest")?;
    ...
    write_one::<PropertyEventData>(out_dir, "PropertyEventData")?;
